...
Product plan from a full scan of `master` on **2026-09-04** (`476a28b`, still `0.1.0`). This supersedes the 3 Sep queue in [#23](https://github.com/theLaxerz/Mainstream/pull/23), plus [#21](https://github.com/theLaxerz/Mainstream/pull/21) / [#19](https://github.com/theLaxerz/Mainstream/pull/19) / [#17](https://github.com/theLaxerz/Mainstream/pull/17).
...
- **Master still has not moved.** Tip is `476a28b` (Mail.app AppleScript timeout, 29 Aug). This is the **fifth** planning day stacked on the same commit. Discovery is done. Shipping is the bottleneck.
- **New finding (missed on 3 Sep):** generic settings IPC only allows `dashboard.layout.v1`. Theme, refresh interval, and any future `onboarding.completed` flag are **silently dropped**. Yesterday’s plan treated refresh as “already persisted.” It is not. Onboarding will not stick until this is fixed.
- **Security:** the 3 Sep afternoon scan failed before opening a PR. [#22](https://github.com/theLaxerz/Mainstream/pull/22) (2 Sep) is still the newest hardening branch.
- **Still not on master:** Tasks ([#15](https://github.com/theLaxerz/Mainstream/pull/15)), four stacked What’s Next docs, five overlapping security PRs.
...
| [#23](https://github.com/theLaxerz/Mainstream/pull/23) / [#21](https://github.com/theLaxerz/Mainstream/pull/21) / [#19](https://github.com/theLaxerz/Mainstream/pull/19) / [#17](https://github.com/theLaxerz/Mainstream/pull/17) | 3 Sep / 2 Sep / 1 Sep / 31 Aug What's Next | Close when this document lands. |
...
`is_generic_setting_key` in `src-tauri/src/security.rs` only matches `dashboard.layout.v1`. The frontend already writes:

- `appearance.theme` (`saveThemePreference`) — catch is empty, so the command-bar cycle looks like it worked until relaunch
- `refresh.intervalMinutes` (`loadRefreshIntervalMinutes`) — always falls back to 15

Yesterday’s plan also wants `onboarding.completed` in the same generic settings table. That key would be denied the same way, so a skip-able overlay would greet every launch.

Allow-list those three keys (and keep secrets on the existing denylist). Extend `setting_keys_are_scoped` so theme / refresh / onboarding pass and `streaming.tmdb_api_key` still fails. Tiny PR; blocks increment 1.
...
- **Wide-grid “Right column”** — at `min-width: 1180px` the grid is 3 columns, but `.placement-right` jumps to `grid-column: 3 / 4`, leaving a hole in column 2. `placementGridStyle()` in `moduleLayout.ts` is unused. Map “right” to the last column honestly, or add a real third placement.
...
- Another planning-only PR while `476a28b` is still tip — close the older What’s Next PRs instead. The only reason this document exists is the settings-allowlist miss that would have broken increment 1.

---

## Suggested sequence for upcoming automations

1. Human: land **#15 Tasks**, land **#22** (or close it), close #14 / #16 / #17 / #18 / #19 / #20 / #21 / #23.
2. **Settings allowlist** (this document’s increment #0) — one-file Rust change. Do not start onboarding without it.
