...
Product plan from a full scan of `master` on **2026-09-02** (`476a28b`, still `0.1.0`). This supersedes the 1 Sep queue in [#19](https://github.com/theLaxerz/Mainstream/pull/19) and the 31 Aug queue in [#17](https://github.com/theLaxerz/Mainstream/pull/17).
...
- **Master did not move.** Tip is still `476a28b` (Mail.app AppleScript timeout, 29 Aug). Three planning days have stacked on the same commit.
- **New open PR:** [#20](https://github.com/theLaxerz/Mainstream/pull/20) (1 Sep afternoon) is now the newest IPC / zero-day hardening branch. It re-applies the unmerged SSRF/input checks and adds RSS/YouTube URL guards, Health zip-slip, OAuth client-ID charset, and a Vite `^7.3.6` floor.
- **Still not on master:** Tasks ([#15](https://github.com/theLaxerz/Mainstream/pull/15)), yesterday’s plan (#19), and four overlapping security PRs.
...
- Today briefing: greeting, next event, unread chats, important email, steps, spend — chips scroll to the matching module
...
| [#20](https://github.com/theLaxerz/Mainstream/pull/20) | IPC / zero-day hardening (1 Sep scan) | Review and land. Newest of the security set. |
| [#18](https://github.com/theLaxerz/Mainstream/pull/18) / [#16](https://github.com/theLaxerz/Mainstream/pull/16) / [#14](https://github.com/theLaxerz/Mainstream/pull/14) | Older copies of the same hardening | Close as duplicates of #20. |
| [#19](https://github.com/theLaxerz/Mainstream/pull/19) / [#17](https://github.com/theLaxerz/Mainstream/pull/17) | 1 Sep / 31 Aug What's Next | Close when this document lands. |
...
Cold launch shows **twelve enabled modules**. Email auto-opens its connect drawer (`setShowSettings(true)` whenever `loadSettings` sees `!connected`). Home, YouTube, Streaming, Mail, Health, and Finance are empty cards. News is the exception — it seeds default feeds *and* fetches RSS on an empty list, so first paint is extra chatty. The hero and briefing cannot carry that.
...
3. **Inbox** — Google / Microsoft / Mail.app cards. Mail.app is timeout-safe; lead with it.
...
`HealthDay.importedAt` is already on the type and in SQLite. The card never shows it. Add “last imported” age and a one-click re-import when the zip is older than a day. Live HealthKit is a later entitlement fight — don’t start it until onboarding + fonts + window chrome are done.
...
- **Double greeting** — `Clock` and `TodayBriefing` both call `greetingFor`. Keep it on the clock readout; let the briefing lead with “At a glance.”
...
1. Human: land **#15 Tasks**, land **#20** (or close it), close #14 / #16 / #17 / #18 / #19.
